...
function renderPaymentTab(c){var cs=(client||{}).settings||{},opts=Array.isArray(cs.paymentOptions)?cs.paymentOptions:[];c.innerHTML='<div class="space-y-6"><div><h2 class="text-xl font-semibold text-white"><i class="fas fa-credit-card text-sprint-400 mr-2"></i>Payment Settings</h2><p class="text-sm text-slate-400 mt-1">Add up to 3 accounts customers can pay after confirming an order.</p></div><form onsubmit="savePaymentSettings(event)" class="space-y-4">'+[0,1,2].map(function(i){return paymentOptionRow(i,opts[i])}).join('')+'<div class="card p-4 border border-yellow-500/20 bg-yellow-500/5"><p class="text-xs text-yellow-200"><i class="fas fa-info-circle mr-1"></i>Only filled rows are saved. Customers see these details only after the system knows the delivery fee.</p></div><button type="submit" class="btn btn-primary text-xs"><i class="fas fa-save"></i> Save Payment Settings</button></form></div>'}
async function savePaymentSettings(e){e.preventDefault();var paymentOptions=[0,1,2].map(function(i){return{method:(document.getElementById('pay-method-'+i)||{}).value||'',accountNumber:((document.getElementById('pay-account-'+i)||{}).value||'').trim(),accountName:((document.getElementById('pay-name-'+i)||{}).value||'').trim()}}).filter(function(row){return row.method&&row.accountNumber&&row.accountName});try{var res=await apiFetch('/api/client/settings',{method:'PUT',body:JSON.stringify({paymentOptions:paymentOptions})});if(res&&res.client){client=res.client}else{client.settings.paymentOptions=paymentOptions}showToast('Payment settings saved.','success');switchClientTab('payment')}catch(err){showToast(err.message,'error')}}
