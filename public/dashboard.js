...
else if(tab==='miniapp')renderLockedTab(c,'Website','globe')
...
function renderProfileTab(c){
  ...
  '<div class="md:col-span-2"><label class="text-xs text-slate-400 block mb-1">Exact Google Maps Link</label><input id="bp-map-url" class="field" value="'+esc(bp.mapUrl||'')+'" placeholder="Paste Google Maps share link for the shop pin"'+(isPending?' disabled':'')+'><p class="text-xs text-slate-500 mt-1">Used by the website address button. If the link contains coordinates, SprintSales also uses them for the Telegram map pin.</p></div>'+
...
var WEBSITE_THEME_PRESETS=[
  {name:'Midnight & Mint',themeColor:'#172b4d',accentColor:'#157b68'},
  {name:'Cobalt & Cyan',themeColor:'#174a7e',accentColor:'#087f8c'},
  {name:'Forest & Brass',themeColor:'#344e41',accentColor:'#866426'},
  {name:'Plum & Rose',themeColor:'#71345f',accentColor:'#a84470'},
  {name:'Charcoal & Gold',themeColor:'#24262d',accentColor:'#8a641f'}
];
...
function websiteHex(value,fallback){var text=String(value||'').trim().toLowerCase();return/^#[0-9a-f]{6}$/.test(text)?text:(fallback||'#173b67')}
function websiteRecommendedTheme(m){var r=(m&&m.recommendedTheme)||{};return{name:r.label||'Your business',description:r.description||'Recommended for your business type.',themeColor:websiteHex(r.themeColor,'#173b67'),accentColor:websiteHex(r.accentColor,'#20a39e')}}
function websiteThemePresets(recommended){var first={name:'Recommended · '+recommended.name,description:recommended.description,themeColor:recommended.themeColor,accentColor:recommended.accentColor,recommended:true};return[first].concat(WEBSITE_THEME_PRESETS.filter(function(item){return item.themeColor!==first.themeColor||item.accentColor!==first.accentColor}))}
function updateWebsiteThemePreview(){
  var main=document.getElementById('miniapp-theme'),accent=document.getElementById('miniapp-accent'),preview=document.getElementById('website-theme-preview');
  if(!main||!accent)return;
  var mainColor=websiteHex(main.value,'#173b67'),accentColor=websiteHex(accent.value,'#20a39e');
  if(preview){preview.style.setProperty('--preview-main',mainColor);preview.style.setProperty('--preview-accent',accentColor)}
  var mainValue=document.getElementById('miniapp-theme-value'),accentValue=document.getElementById('miniapp-accent-value');
  if(mainValue)mainValue.textContent=mainColor.toUpperCase();
  if(accentValue)accentValue.textContent=accentColor.toUpperCase();
  document.querySelectorAll('[data-website-palette]').forEach(function(button){
    button.classList.toggle('selected',button.getAttribute('data-main')===mainColor&&button.getAttribute('data-accent')===accentColor);
  });
}
function syncWebsiteThemeColor(kind,value,customized){
  var color=websiteHex(value,'');
  if(!color)return;
  var picker=document.getElementById(kind==='accent'?'miniapp-accent':'miniapp-theme');
  var hex=document.getElementById(kind==='accent'?'miniapp-accent-hex':'miniapp-theme-hex');
  if(picker)picker.value=color;
  if(hex)hex.value=color.toUpperCase();
  var flag=document.getElementById('miniapp-theme-customized');
  if(flag&&customized!==undefined)flag.value=customized?'true':'false';
  updateWebsiteThemePreview();
}
function useWebsitePalette(main,accent,recommended){
  syncWebsiteThemeColor('theme',main,!recommended);
  syncWebsiteThemeColor('accent',accent,!recommended);
  var flag=document.getElementById('miniapp-theme-customized');
  if(flag)flag.value=recommended?'false':'true';
  updateWebsiteThemePreview();
}
function websiteThemeEditor(themeColor,accentColor,recommended,customized){
  var presets=websiteThemePresets(recommended);
  return '<section class="website-theme-editor"><div class="website-theme-heading"><div><span class="website-theme-kicker">Website colors</span><h4>Make the shop feel like your business</h4><p>Your recommended colors are selected from your business type. Choose another palette or fine-tune each color.</p></div><button type="button" class="btn btn-ghost text-xs" onclick="useWebsitePalette(\''+esc(recommended.themeColor)+'\',\''+esc(recommended.accentColor)+'\',true)"><i class="fas fa-wand-magic-sparkles"></i> Use recommended</button></div>'+
  '<div id="website-theme-preview" class="website-theme-preview" style="--preview-main:'+esc(themeColor)+';--preview-accent:'+esc(accentColor)+'"><div class="website-preview-bar"><span class="website-preview-logo">'+esc(String((client||{}).businessName||'S').charAt(0).toUpperCase())+'</span><strong>'+esc((client||{}).businessName||'Your shop')+'</strong><i class="fas fa-magnifying-glass"></i></div><div class="website-preview-body"><span>New arrivals</span><h5>Products your customers will love</h5><button type="button" tabindex="-1">Shop now</button></div></div>'+
  '<div class="website-palette-grid">'+presets.map(function(item){return'<button type="button" class="website-palette-option '+(item.themeColor===themeColor&&item.accentColor===accentColor?'selected':'')+'" data-website-palette data-main="'+esc(item.themeColor)+'" data-accent="'+esc(item.accentColor)+'" onclick="useWebsitePalette(\''+esc(item.themeColor)+'\',\''+esc(item.accentColor)+'\','+(item.recommended?'true':'false')+')"><span class="website-palette-swatches"><i style="background:'+esc(item.themeColor)+'"></i><i style="background:'+esc(item.accentColor)+'"></i></span><span><b>'+esc(item.name)+'</b>'+(item.recommended?'<small>Best fit</small>':'')+'</span><i class="fas fa-check website-palette-check"></i></button>'}).join('')+'</div>'+
  '<div class="website-color-controls"><label class="website-color-control"><input id="miniapp-theme" type="color" value="'+esc(themeColor)+'" oninput="syncWebsiteThemeColor(\'theme\',this.value,true)"><span><b>Main color</b><small>Headers, navigation, and strong actions</small><strong id="miniapp-theme-value">'+esc(themeColor.toUpperCase())+'</strong></span><input id="miniapp-theme-hex" class="field" value="'+esc(themeColor.toUpperCase())+'" maxlength="7" aria-label="Main color hex value" onchange="syncWebsiteThemeColor(\'theme\',this.value,true)"></label>'+
  '<label class="website-color-control"><input id="miniapp-accent" type="color" value="'+esc(accentColor)+'" oninput="syncWebsiteThemeColor(\'accent\',this.value,true)"><span><b>Accent color</b><small>Highlights, prices, and selected controls</small><strong id="miniapp-accent-value">'+esc(accentColor.toUpperCase())+'</strong></span><input id="miniapp-accent-hex" class="field" value="'+esc(accentColor.toUpperCase())+'" maxlength="7" aria-label="Accent color hex value" onchange="syncWebsiteThemeColor(\'accent\',this.value,true)"></label></div>'+
  '<input id="miniapp-theme-customized" type="hidden" value="'+(customized?'true':'false')+'"></section>';
}
async function chooseMiniappTemplate(value){var selected=normalizeMiniappTemplate(value),input=document.getElementById('miniapp-template');if(input)input.value=selected;document.querySelectorAll('[data-miniapp-design]').forEach(function(card){var active=card.getAttribute('data-miniapp-design')===selected;card.classList.toggle('selected',active);var button=card.querySelector('[data-choose-design]');if(button){button.classList.toggle('btn-primary',active);button.classList.toggle('btn-secondary',!active);button.innerHTML=active?'<i class="fas fa-spinner loading-spinner"></i> Saving':'Use this design';button.disabled=active}});try{var current=(((client||{}).settings||{}).miniapp)||{};var miniapp={enabled:current.enabled!==false,slug:miniappSlug(current.slug||miniappDefaultSlug()),customDomain:miniappCleanDomain(current.customDomain||''),template:selected,themeColor:current.themeColor||'#173b67',accentColor:current.accentColor||'#20a39e',themeCustomized:current.themeCustomized===true};var res=await apiFetch('/api/client/settings',{method:'PUT',body:JSON.stringify({miniapp:miniapp})});if(res&&res.client){client=res.client}else{client.settings=client.settings||{};client.settings.miniapp=miniapp}showToast('Website design saved and published.','success');switchClientTab('miniapp')}catch(err){showToast(err.message||'Could not save the website design.','error');switchClientTab('miniapp')}}
...
function renderMiniappTab(c){
  var cs=(client||{}).settings||{},m=cs.miniapp||{},slug=miniappSlug(m.slug||cs.storeSlug||(client||{}).businessName||(client||{}).id),enabled=m.enabled!==false,template=normalizeMiniappTemplate(m.template||'clean-retail'),theme=websiteHex(m.themeColor,'#173b67'),accent=websiteHex(m.accentColor,'#20a39e'),recommended=websiteRecommendedTheme(m),themeCustomized=m.themeCustomized===true,custom=m.customDomain||cs.miniappDomain||'',productCount=(appState.products||[]).filter(function(p){return p.isActive!==false}).length,isCake=clientIsCakeBusiness(cs),url=miniappUrlForSlug(slug,custom);
  c.innerHTML='<div class="space-y-6"><div class="flex flex-col lg:flex-row lg:items-end justify-between gap-3"><div><h2 class="text-xl font-semibold text-white"><i class="fas fa-globe text-sprint-400 mr-2"></i>Website</h2><p class="text-sm text-slate-400 mt-1">Your public online shop, powered by the same products, orders, delivery, and payment settings in SprintSales.</p></div><a id="miniapp-preview-link" href="'+esc(url)+'" target="_blank" rel="noopener" class="btn btn-secondary text-xs"><i class="fas fa-arrow-up-right-from-square"></i> Open Website</a></div>'+miniappDesignPicker(template,isCake)+
  '<div class="grid grid-cols-1 xl:grid-cols-[1fr_.8fr] gap-5"><form onsubmit="saveMiniappSettings(event)" class="card p-5 space-y-5"><div><h3 class="text-white font-semibold">Website Address</h3><p class="text-xs text-slate-500 mt-1">Control where customers find your shop and how it looks.</p></div><label class="flex items-center gap-2 text-sm text-white"><input id="miniapp-enabled" type="checkbox" '+(enabled?'checked':'')+'> Website is open to customers</label><div><label class="text-xs text-slate-400 block mb-1">Shop link</label><div class="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-2"><input id="miniapp-slug" class="field" value="'+esc(slug)+'" placeholder="aman-electronics" oninput="this.value=miniappSlug(this.value);updateMiniappPreview()"><button type="button" class="btn btn-ghost text-xs" onclick="document.getElementById(\'miniapp-slug\').value=miniappDefaultSlug();updateMiniappPreview()"><i class="fas fa-wand-magic-sparkles"></i> Use business name</button></div><p class="text-xs text-slate-500 mt-1">Your SprintSales address will look like https://'+esc(slug||'your-shop')+'.sprintsales.net.</p></div><div><label class="text-xs text-slate-400 block mb-1">Public website address</label><input id="miniapp-preview-text" class="field" readonly value="'+esc(url)+'"></div><div><label class="text-xs text-slate-400 block mb-1">Your own domain <span class="text-slate-500">(optional)</span></label><input id="miniapp-domain" class="field" value="'+esc(custom)+'" placeholder="shop.yourbusiness.com" oninput="updateMiniappPreview()"><p class="text-xs text-slate-500 mt-1">Connect a domain you own after its DNS points to SprintSales.</p></div>'+websiteThemeEditor(theme,accent,recommended,themeCustomized)+'<button type="submit" class="btn btn-primary text-xs"><i class="fas fa-save"></i> Save Website Settings</button></form>'+
  '<div class="space-y-4"><div class="card p-5"><h3 class="text-white font-semibold mb-3">Connected automatically</h3><div class="space-y-3 text-sm text-slate-300"><div class="flex gap-3"><i class="fas fa-box text-sprint-400 mt-1"></i><p><b class="text-white">'+esc(productCount)+'</b> active item'+(productCount===1?'':'s')+' from Products.</p></div><div class="flex gap-3"><i class="fas fa-image text-sprint-400 mt-1"></i><p>Public product photos and featured selections.</p></div><div class="flex gap-3"><i class="fas fa-tag text-sprint-400 mt-1"></i><p>Prices, categories, choices, discounts, delivery fees, and payment accounts.</p></div><div class="flex gap-3"><i class="fas fa-cart-shopping text-sprint-400 mt-1"></i><p>Orders, customer details, payments, and tracking stay synchronized with this dashboard.</p></div></div></div><div class="card p-5 border border-blue-500/20 bg-blue-500/5"><h3 class="text-white font-semibold mb-2">One shop, two doors</h3><p class="text-sm text-slate-400">Customers can shop from the website or Telegram. Their activity and orders continue through the same SprintSales system.</p></div></div></div></div>';
  setTimeout(function(){updateMiniappPreview();updateWebsiteThemePreview()},0);
}
async function saveMiniappSettings(e){
  e.preventDefault();
  var miniapp={enabled:document.getElementById('miniapp-enabled').checked,slug:miniappSlug(document.getElementById('miniapp-slug').value||miniappDefaultSlug()),customDomain:(document.getElementById('miniapp-domain').value||'').trim().toLowerCase().replace(/^https?:\/\//,'').replace(/^www\./,'').split('/')[0],template:document.getElementById('miniapp-template').value,themeColor:websiteHex(document.getElementById('miniapp-theme').value,'#173b67'),accentColor:websiteHex(document.getElementById('miniapp-accent').value,'#20a39e'),themeCustomized:(document.getElementById('miniapp-theme-customized').value==='true')};
  try{var res=await apiFetch('/api/client/settings',{method:'PUT',body:JSON.stringify({miniapp:miniapp})});if(res&&res.client){client=res.client}else{client.settings=client.settings||{};client.settings.miniapp=miniapp}showToast('Website settings saved and published.','success');switchClientTab('miniapp')}catch(err){showToast(err.message,'error')}
}
